...
void loop() 
{
  if ( cnt_cycles >= cnt_cycles_max )
  {
    air ( );
    
    update_pl600_outputs ( );
    if ( (io_pl600.out & 0x01) != 0 )
    {
      u8x8.setCursor ( 0, 4 );
      u8x8.clearLine ( 4 );
      u8x8.print( "re-enter RUN_MODE for next run" );
    }
    
    do
    {
      update_pl600_outputs ( );
    } while( (io_pl600.out & 0x01) != 0 ); // wait for RUN_MODE == 0;
    
    u8x8.setCursor ( 0, 4 );
    u8x8.clearLine ( 4 );
    u8x8.print( "press BALANCE and go to RUN_MODE" );
  
    do
    {
      update_pl600_outputs ( );
    } while( (io_pl600.out & 0x01) == 0 ); // wait for RUN_MODE == 1;
    ...
    u8x8.println("Gen.");
    u8x8.setCursor ( 0, 5 );
    u8x8.println("Parts total: ");
    u8x8.println("Parts good:  "); 
    u8x8.println("Parts bad:   ");
    u8x8.setCursor ( 24, 4 );
    u8x8.println("Eval.");
    u8x8.setCursor ( 23, 5 );
    u8x8.print( "|" );  
