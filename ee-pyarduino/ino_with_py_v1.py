...
class runIno:
    ...
    def read_memory(self, position:int, length:int, data:list[int])-> list[int]:
        ...
        if (position +length ) > 128:
            raise Exception ("Your length '{}'is longer than the available memory space!".format(length))
        self.ser.timeout=1
        space=" "
    ...
    def write_memory(self, position:int, data:list[int], check:bool=True) -> bool:   
        ...
        for d in cont:
            dat += str(d)+space
        cmd="00"+space+ space + str(hex(position))+ space +str(hex(length))+ space + dat
...
# content = [0x01, 0x02, 0x8C, 0xB0, 0x57, 0x4D, 0x74, 0x67, 0x2E, 0xEE, 0x3E, 0x15, 0x42, 0xA3, 0x01, 0xCB, 0x41, 0x42, 0x43, 0x44, 0x30, 0x36, 0x00, 0x00, 0x44, 0x65, 0x66, 0x61, 0x75, 0x6C, 0x74, 0x5F]
content2=[0x01, 0x02, 0x8C, 0xB0, 0x57, 0x4D, 0x74, 0x67, 0x2E, 0xEE, 0x3E, 0x15, 0x42, 0xA3, 0x01, 0xCB]
        #[0x01, 0x02, 0x8C, 0xB0, 0x57, 0x4D, 0x74, 0x67, 0x2E, 0xEE, 0x3E, 0x15, 0x42, 0xA3, 0x01, 0xCB]
run_file.write_memory(0,content2)
# da=[]
# # print(run_file.write_memory(0,content2))
# data:int=run_file.read_memory(0,4,da)

# print(run_file.read_memory(0,4,data))
# print(data)
# if data==[1, 2, 140, 176]:
#     print("doch richtis")
# else:
#     print("pas bon xa")
# # # c = run_file.clear_memory()
# # print("c: "+ str(c))
